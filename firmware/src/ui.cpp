...
// Usage counts as "live" within this window. Long by design: the daemon is
// lifecycle-bound (a watcher runs it only while an AI CLI is active), so when it
// stops the device should keep showing the last-synced numbers as a glanceable
// snapshot rather than dropping to the idle screen. During active use the daemon
// sends every ~60s, so this generous window never hides a genuinely stuck feed —
// and the watcher respawns a dead daemon within ~10s anyway.
static const uint32_t DATA_FRESH_MS = 43200000;  // 12h
